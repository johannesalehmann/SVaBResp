...
pub struct ComputeResponsibilityCommand {
    model: String,
    property: String,
    constants: String,
...
impl ComputeResponsibilityCommand {
    ...
    pub fn execute(self) {
        ...
        let model_description = ModelFromFile::new(self.model.clone(), self.property.clone());
        self.execute_with_model_description(model_description);
    }

    fn execute_with_model_description<M: ModelAndPropertySource>(self, model_description: M) {
        match self.grouping {
            GroupingKind::Individual => self.execute_with_grouping_scheme(
                model_description,
                IndividualGroupExtractionScheme::new(),
            ),
            GroupingKind::Labels { ref labels } => {
                let labels = labels.clone();
                self.execute_with_grouping_scheme(
                    model_description,
                    LabelGroupExtractionScheme::new(labels),
                )
            }
            GroupingKind::Modules => self.execute_with_grouping_scheme(
                model_description,
                ModuleGroupExtractionScheme::new(),
            ),
            GroupingKind::Actions => self.execute_with_grouping_scheme(
                model_description,
                ActionGroupExtractionScheme::new(),
            ),
    ...
    fn execute_with_grouping_scheme<M: ModelAndPropertySource, G: GroupExtractionScheme>(
        ...
        match self.algorithm {
            AlgorithmKind::BruteForce => self.execute_with_algorithm(
                model_description,
                grouping_scheme,
            ...
            AlgorithmKind::Stochastic => {
                panic!("Stochastic algorithm not implemented in self yet")
            }
            AlgorithmKind::Refinement => match self.refinement_initial_partition {
                RefinementInitialPartition::Singleton => self
                    .execute_with_initial_partition_provider(
                        model_description,
                        grouping_scheme,
                        SingletonInitialPartition::new(),
                    ),
                RefinementInitialPartition::Random { block_count } => self
                    .execute_with_initial_partition_provider(
                        model_description,
                        grouping_scheme,
    ...
    >(
        ...
        match self.refinement_block_selection {
            RefinementBlockSelection::Random { block_count } => self
            ...
            RefinementBlockSelection::MaxDelta { block_count } => self
                .execute_with_block_selection_heuristics(
                    model_description,
                    grouping_scheme,
                    initial_partition_provider,
            ...
            RefinementBlockSelection::MinDelta { block_count } => self
                .execute_with_block_selection_heuristics(
                    model_description,
                    grouping_scheme,
                    initial_partition_provider,
            ...
            RefinementBlockSelection::MinFrontier { block_count } => self
                .execute_with_block_selection_heuristics(
                    model_description,
                    grouping_scheme,
                    initial_partition_provider,
    ...
    >(
        ...
        match self.refinement_splitting {
            RefinementSplitting::Random => self.execute_with_block_splitting_heuristics(
                model_description,
                grouping_scheme,
                initial_partition_provider,
            ...
            RefinementSplitting::FrontierRandom => self.execute_with_block_splitting_heuristics(
                model_description,
                grouping_scheme,
                initial_partition_provider,
                block_selection_heuristics,
            ...
            RefinementSplitting::FrontierMostEdgesToWinningAndLosing => self
            ...
            RefinementSplitting::FrontierMostEdgesToWinning => self
                .execute_with_block_splitting_heuristics(
                    model_description,
                    grouping_scheme,
                    initial_partition_provider,
            ...
            RefinementSplitting::FrontierMostEdgesToLosing => self
                .execute_with_block_splitting_heuristics(
                    model_description,
                    grouping_scheme,
                    initial_partition_provider,
